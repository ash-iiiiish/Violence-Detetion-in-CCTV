...
WEAPON_RELAX_FRAMES = 30   # <-- prevents flickering
VIOLENCE_SMOOTH_COUNT = 5  # <-- smoothing window

# 🔥 MANUAL PATHS (UNCHANGED)
MODEL_PATH = "C:/Users/kumar/OneDrive/Desktop/TRY-3/Violence-Detetion-in-CCTV/violence-app/backend/live_violence.pth"
YOLO_PATH = "C:/Users/kumar/OneDrive/Desktop/TRY-3/Violence-Detetion-in-CCTV/violence-app/backend/best-yolo.pt"
...
def predict_video(video_path):
    ...
    weapon_mode_activated = False   # 🔥 permanent flag
    ...
    while True:
            ...
            for r in results:
                for box in r.boxes:
                    ...
                    if conf > WEAPON_CONF_THRESHOLD:
                        ...
                        weapon_mode_activated = True  # 🔥 permanent activation
                        cls_id = int(box.cls[0])
                        class_name = weapon_model.names[cls_id]
                        x1, y1, x2, y2 = map(int, box.xyxy[0])
                        weapon_boxes.append((x1, y1, x2, y2, conf, class_name))
        ...
        if len(frame_buffer) == CLIP_LEN:
            ...
            most_common = Counter(violence_history).most_common(1)[0][0]

            violence_label = most_common
            violence_conf = conf.item()
        else:
        ...
        if weapon_mode_activated:
            ...
            final_confidence = violence_conf
            label_color = (0, 165, 255) if violence_label != "NonFight" else (0, 255, 0)

        # ================= DRAW WEAPON BOXES =================
        for (x1, y1, x2, y2, conf, class_name) in weapon_boxes:
